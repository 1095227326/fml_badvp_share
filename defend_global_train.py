...
def parse_option():
    ...
    t_save_path = './save/defend_global_{}_{}_{}_{}_{}_{}'
...
def sample_balanced_data(data, labels, samples_per_class):
    # 将数据和标签转换为NumPy数组（如果它们还不是）
    data = np.array(data)
    ...
    # 对每个类别进行循环，从每个类别中随机选择samples_per_class个样本
    for label in unique_labels:
        # 找到当前类别的所有索引
        indices = np.where(labels == label)[0]
        
        # 如果当前类别的样本数少于要求的样本数，则重复选择，否则随机选择
        if len(indices) < samples_per_class:
            chosen_indices = np.random.choice(indices, size=samples_per_class, replace=True)
        else:
            chosen_indices = np.random.choice(indices, size=samples_per_class, replace=False)
        
        # 添加选定的数据和标签
        sampled_data.append(data[chosen_indices])
        sampled_labels.extend([label] * samples_per_class)
    
    # 合并所有选定的数据和标签，并转换为适当的格式
...
def inti_train_data(args):
    ...
    num_samples_per_class = max(10,int(len(o_train_labels) // len(np.unique(o_train_labels)) * 0.01))

    selected_data, selected_labels = sample_balanced_data(o_train_data, o_train_labels, num_samples_per_class)
    print(selected_data.shape,selected_labels)
    # display_images_with_labels(selected_data[:100],selected_labels[:100],'g_train',save_path='imgs/{}'.format(args.dataset))
    gloabal_train_datas = (selected_data, selected_labels,[])
    ...
    return final_local_train_datas,test_datas,poison_pairs,subset_realidx_list,gloabal_train_datas
...
def train(indices, train_loader, model, prompter, optimizer, scheduler, criterion, epoch,args):
    ...
    for i, (images, target) in enumerate(tqdm(train_loader,disable = True)):
        # print(target)
...
def main(args):
    ...
    final_local_train_datas,test_datas,poison_pairs,subset_realidx_list,global_train_datas = inti_train_data(args) 
    ...
    for i in range(args.round):
        ...
        global_data,global_lables,global_tags = global_train_datas
        display_images_with_labels(global_data[:100],global_lables[:100],'g_train',save_path='imgs/{}'.format(args.dataset))

        global_train_dataset = New_Data.CustomDataset(global_data,global_lables,global_tags)
        global_train_loader = DataLoader(global_train_dataset,batch_size=batch_size,shuffle=True,num_workers=num_workers,pin_memory=True)
        for now_epoch in range(args.epochs):
            loss,top1 = train(indices,global_train_loader,model,global_node.prompter,global_node.optimizer,global_node.scheduler,
                              global_node.criterion,i*args.epochs+now_epoch,args)
            desc = 'Round {}/{} Global Node Epoch {} Loss is {:4.5f}'
            print(desc.format(i+1, args.round, now_epoch + 1, loss))
